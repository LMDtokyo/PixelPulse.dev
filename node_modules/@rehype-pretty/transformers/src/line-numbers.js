 ...
 * })
 * ```
 */ export function transformerLineNumbers(options = {
  autoApply: true
}) {
  return {
    ...
    pre (node) {
      ...
      const startLineNumberMeta = metaStrings?.find((s)=>s.startsWith('startLineNumber='));
      const startLineNumber = (Number(startLineNumberMeta?.split('=')?.at(1)) || 1) - 1;
      if (startLineNumber) {
